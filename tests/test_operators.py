...
def test_other(a: float) -> None:
    ...
    # монотонность: relu не убывает
    assert relu(a) <= relu(a + 1.0)

    # нечётность: neg(-a) равно -neg(a)
    assert_close(neg(-a), -neg(a))

    # чётность: квадрат не зависит от знака
    assert_close(mul(a, a), mul(-a, -a))
